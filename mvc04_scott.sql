...
--○ 뷰(REGIONVIEW) 조회를 통한 지역 검색 쿼리문 구성
SELECT REGIONID, REGIONNAME, DELCHECK
FROM REGIONVIEW
WHERE REGIONID=1
ORDER BY REGIONID;
...
--○ 뷰(DEPARTMENTVIEW) 조회를 통한 부서 검색 쿼리문 구성
SELECT DEPARTMENTID, DEPARTMENTNAME, DELCHECK
FROM DEPARTMENTVIEW
WHERE DEPARTMENTID=1
ORDER BY DEPARTMENTID;
...
--○ 뷰(POSITIONVIEW) 조회를 통한 직위 검색 쿼리문 구성
SELECT POSITIONID, POSITIONNAME, MINBASICPAY, DELCHECK
FROM POSITIONVIEW
WHERE POSITIONID=1
ORDER BY POSITIONID;
--> 한 줄 구성 
SELECT POSITIONID, POSITIONNAME, MINBASICPAY, DELCHECK FROM POSITIONVIEW WHERE POSITIONID=1 ORDER BY POSITIONID
;
